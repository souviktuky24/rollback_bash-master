...
remove_prefix(){
	echo "$string" | sed 's/_//' | sed 's/-//' | sed 's/\.//'
}
...
create_build(){
	...
	if (( $? == 0 )) ;then
		...
		ln -s release_"$version1".1.0_dummy current_release
		current_rel=release_"$version1".1.0_dummy
	fi
...
restore_state(){
	...
	do
		echo "hehe"
		local dele_name="$(remove_prefix $REF_DEL)"
		echo $dele_name
		if [ -d "$REF_DEL" ] 
		then
			rm -rf $REF_DEL
